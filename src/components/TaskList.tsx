...
export default function TaskList({ bg, border, title, tableName, tasks }: { bg: string, border: string, title: string, tableName: string, tasks: Task[] }) {
    ...
    return (
        <div className={`flex flex-col rounded-lg border-1 ${border} flex-1`}>
            ...
            {isLoading ? (
                ...
                tasks.length > 0 ? (
                    tasks.map((task) => (
                        <ListItem key={task.id} task={task} border={border} bg={bg} completed={completedTasks.includes(task.id)} callback={handleTaskCompletion} />
                    ))
                ) : (
